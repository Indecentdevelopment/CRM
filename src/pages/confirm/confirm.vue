<template>
	<div class="confirm">
		...
		<div class="car-info" v-if="carInfo.carType">
			...
			<div class="info">
				<div class="name">
					{{carInfo.carType.carSeries.brandName.replace(/^[A-Z] - /g,'')}} {{carInfo.carType.carSeries.brandType}} {{carInfo.carType.carSeries.seriesName}} {{carInfo.carType.displacement}} {{carInfo.carType.productiveYear}} 年生产 ({{carInfo.carNumber}}/{{carInfo.mileage}}km/ {{carInfo.monthMileage}}km)
				</div>
				<div class="config">
					采用{{carInfo.carType.cylinder}} 缸发动机,机油用量 {{carInfo.carType.engineOil}}L 防冻液用量 {{carInfo.carType.antifreezing}}L
				</div>
			</div>
		</div>
		<!-- 轮胎服务 -->
							...
							<div class="productList" :class="{active: categorys.active}" v-show="categorys.active">
								...
								<div class="service-list" v-if="categorys.serviceList.length>0">
									...
									<div class="preferentialPolicys" v-if="categorys.preferentialPolicys.length>0">
										<div class="item" v-for="(policy, polIndex) in categorys.preferentialPolicys" :key="policy.id" :class="{active: policy.active}" @click="policyClick(proIndex, catIndex, polIndex)">
											{{policy.name}}
										</div>
									</div>
									<div class="service-box">
										<div class="item" v-for="(service, serIndex) in categorys.serviceList" :key="service.id" :class="{'active check': service.active}" @click="serviceClick(proIndex, catIndex, serIndex)" :data-id="service.id" :data-num="service.selectQuantity">
						...
						<div v-else>
							...
							<div class="" v-for="(categorys2, cat2Index) in categorys.childCategorys" :key="categorys2.id">
								...
								<div class="productList" :class="{active: categorys.active}" v-show="categorys.active">
									...
									<div class="product-box">
										<div class="item" v-for="(goods, goodsIndex) in categorys2.productList" @click="productClick(goodsIndex, proIndex, catIndex, cat2Index)" :key="goods.id" :class="{'active check': goods.active}" :data-id="goods.id" :data-num="goods.selectQuantity">
											<!-- 商品信息 -->
											<div class="goods">
